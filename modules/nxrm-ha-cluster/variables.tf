...
variable "nxrm_name" {
  description = "Helpful friendly name for this NXRM Cluster (min 4 alphanumeric characters)"
  type        = string
  validation {
    condition     = length(regex("[[:alnum:]]{4,}", var.nxrm_name)) > 4
    error_message = "Name for this NXRM must be 4 or more alphanumeric characters."
  }
}
...
variable "db_username" {
  description = "Username for NXRM to use to access PostgreSQL service."
  type        = string
  default     = null
  sensitive   = true
}
...
variable "db_password" {
  description = "Password for NXRM to use to access PostgreSQL service."
  type        = string
  default     = null
  sensitive   = true
}
